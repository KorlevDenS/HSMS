...
public class DesertSimulationService implements SimulationApi {
    ...
    public DesertSimulationResult runDesertScenario(String actorLogin, DesertSimulationRequest request) {
        ...
        missionRequest.setTitle("Симуляция пустыни " + now);
        missionRequest.setZoneId(scenario.zoneId());
        missionRequest.setHarvesterId(scenario.harvesterId());
        missionRequest.setCrewId(scenario.crewId());
        missionRequest.setPlannedStart(now.plusSeconds(600));
        missionRequest.setPlannedEnd(now.plusSeconds(4 * 3600L));
        missionRequest.setRoute(route);
